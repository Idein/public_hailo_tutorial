...
class YoloXLayer():
    ...
    def __init__(
        ...
        self.stride = [(in_size[0] / o[0], in_size[1] / o[1]) for o in output_sizes]
        self.grid = [
            self.make_grid(nx, ny)
            for ny, nx in output_sizes
        ]
    ...
    def run(self, preds):
        ...
        for i in range(self.n_output):
            ...
            stride_y, stride_x = self.stride[i]
            xy = (x[:, :2] + grid) * np.array([stride_x, stride_y])
            wh = np.exp(x[:, 2:4]) * np.array([stride_x, stride_y])
        ...
        if len(boxes) == 0:
            return np.empty((0, 4)), np.empty((0, 1)), np.empty((0, 1))
